...
`www/` was ported from a separately maintained static PWA ([gitagyan.github.io](https://gitagyan.in)) to fit this repo's Capacitor structure. That source also contained ~700 pre-rendered per-chapter/per-verse static HTML pages built for GitHub Pages SEO (plus a Hindi mirror under `hi/`) — those are **not** included here, since this app serves everything through the single `index.html` SPA with a catch-all rewrite (same as the `www/` used in the sibling `muktmuni-app` and `vachanamrut-app` repos). If SEO-optimized static pages for `gitagyan.in` are still wanted, keep maintaining those in the original GitHub Pages repo separately — it's a different serving model than this one.

The original site also had a newer, in-progress React/Vite rewrite (with server-side Gemini AI endpoints via Firebase Functions) previously pushed to this repo. That version was **not** carried forward — this repo now ships the vanilla-JS PWA instead, matching the architecture of the other Capacitor apps in this family. If the AI guidance feature is wanted again later, it needs to be re-added deliberately (client-side key entry, like the original static site's design, or a server-side Cloud Function, like the React rewrite's design — pick one and be consistent about where the API key lives).
...
Editing `www/` (verse text, styling, etc.) doesn't need a new App Store
...
`www/assets/verse_recitation/` holds ~112MB of MP3 Sanskrit recitation audio
for offline in-app playback, bundled straight into both native shells (per
the architecture decision for this app — same approach as the bundled audio
in the `vachanamrut-app` repo). Combined with `assets/gita.db` (~15MB) and
`images/` (~35MB), expect long `npx cap sync` copies and a sizeable install
(~200MB) on both stores. If that ever becomes a problem, the fix is
switching audio playback in `js/app.js` to an absolute URL pointing at the
Firebase-hosted `www/` instead of the bundled relative path, and dropping
`assets/verse_recitation/` from the native copies.
...
├── resources/                # Source icon/splash art for `npx @capacitor/assets generate`
└── package.json
```
...
2. To see changes in the native apps, sync and rebuild:
   ...
   npx cap sync
   ```
   You can also sync a single platform: `npx cap sync ios` or `npx cap sync android`. Use `npx cap copy` instead if you only changed web assets and haven't touched Capacitor plugins/config — it's faster since it skips the native dependency update step.
...
2. In Xcode, select the **App** target → **Signing & Capabilities**:
   - Choose your Team.
   - Set a unique bundle identifier if you don't have access to `com.bhagvadgeeta.app`.
...
3. Create a release keystore (one-time, keep it **outside of git** and back it up — losing it means you can never update the app again under the same listing):
   ```bash
   keytool -genkey -v -keystore bhagvadgeeta-release.keystore \
     -alias bhagvadgeeta -keyalg RSA -keysize 2048 -validity 10000
   ```
...
- [ ] Confirm the ~150-200MB bundled audio/db/images are acceptable for both stores' size guidance, or switch to remote streaming (see [A note on app size](#a-note-on-app-size)).
